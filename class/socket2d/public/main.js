...
socket.on('newplayer', function(data) {
  playerNumbers.push(new Player(120, data.ypos));
})
...
socket.on('currentPlayers', function(data) {
  ...
  numberOfPlayers.forEach((each)=> {
    playerNumbers.push(new Player(each.xPos, each.yPos, each.userId));

  });
  // playerNumbers.push(new Player(numberOfPlayers.for))
});
